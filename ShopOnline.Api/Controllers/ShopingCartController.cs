...
namespace ShopOnline.Api.Controllers
{
    ...
    public class ShopingCartController : ControllerBase
    {
        ...
        public async Task<ActionResult<CartItemDto>> PostItem([FromBody] CartItemToAddDto cartItemToAddDto)
        {
            ...
            try
            {
                var newCartItem = await this.shoppingCartRepository.AddCartItem(cartItemToAddDto);
                if (newCartItem == null)
                {
                    return NoContent();
                }
                ...
                if (product == null)
                {
                    throw new Exception($"Something went wrong when attempting to retreive product (product Id: ({cartItemToAddDto.ProductId}))");
                }
